...
class LiteLLMMCPServer {
  ...
  async chatCompletion(args) {
    ...
    try {
      ...
      if (provider === 'anthropic') {
        const result = await this.callAnthropicAPI(selectedModel, messages, api_key, max_tokens, temperature);
        response = result.response;
        usage = result.usage;
      } else if (provider === 'google') {
        const result = await this.callGoogleAPI(selectedModel, messages, api_key, max_tokens);
        response = result.response;
        usage = result.usage;
      } else {
        // OpenAI and compatible providers
        const effectiveBaseUrl = base_url || 'https://api.openai.com/v1';
        const client = this.initializeClient(effectiveBaseUrl, api_key);
        ...
        if (selectedModel.startsWith('gpt-5') && max_completion_tokens) {
  ...
  async callAnthropicAPI(model, messages, apiKey, maxTokens, temperature) {
  ...
  async callGoogleAPI(model, messages, apiKey, maxTokens) {
    const contents = messages.map(msg => ({
      parts: [{ text: msg.content }],
      role: msg.role === 'assistant' ? 'model' : 'user'
    }));

    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${this.mapToGoogleModel(model)}:generateContent?key=${apiKey}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
